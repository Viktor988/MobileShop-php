...
$status=0;
$statuss=3;
$upit=$konekcija->prepare("SELECT* FROM meni where status=:statuss or status=:statusss");
...
$status2=1;
$status3=3;
$upit=$konekcija->prepare("SELECT* FROM meni where status=:status0 or status=:status1  or status=:status2");
...
echo $ispis;
  ...
  $status=0;
  $stat=2;
$upit=$konekcija->prepare("SELECT* FROM meni where status=:statuss or status=:broj");
$sve=$upit->bindParam(":statuss",$status);
